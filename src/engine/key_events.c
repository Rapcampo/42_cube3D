...
/*   Updated: 2025/06/21 20:53:06 by tialbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_mov(t_player *p)
{
	...
	if (xmov != 0
		&& map_coord(floor(p->pos.x + xform * -p->dir.y), floor(p->pos.y)) == CORR)
		p->pos.x += xform * -p->dir.y;
	if (xmov != 0
		&& map_coord(floor(p->pos.x), floor(p->pos.y + xform * p->dir.x)) == CORR)
		p->pos.y += xform * p->dir.x;
	if (ymov != 0
		&& map_coord(floor(p->pos.x + yform * p->dir.x), floor(p->pos.y)) == CORR)
		p->pos.x += yform * p->dir.x;
	if (ymov != 0
		&& map_coord(floor(p->pos.x), floor(p->pos.y + yform * p->dir.y)) == CORR)
